...
import { IconCirclePlus, IconSearch, IconUsersGroup, IconPencil, IconTrash } from '@tabler/icons-vue';
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between w-full flex-wrap gap-3">
        <h2 class="font-bold text-xl text-slate-900 tracking-tight leading-tight">User Administration</h2>
        <button
      ...
      <div class="flex flex-wrap items-end gap-3">
        <div class="flex-1 min-w-[220px]">
          ...
          <div class="relative">
            ...
            <input
              id="user-search" v-model="searchForm.search" type="text" placeholder="Name or email…"
        ...
        <div>
          ...
          <select
            id="user-status" v-model="searchForm.status"
    ...
    <div class="dict-card overflow-hidden">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="user in users.data" :key="user.id" class="hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-4 text-right whitespace-nowrap space-x-3">
                <button class="text-sm text-accent-500 hover:text-accent-600 hover:underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 rounded font-medium inline-flex items-center gap-1 transition-colors" @click="openEdit(user)">
                  <IconPencil class="w-4 h-4" /> Edit
                </button>
                <button class="text-sm text-red-600 hover:text-red-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/40 rounded font-medium inline-flex items-center gap-1 transition-colors" @click="deleting = user">
      ...
      <div v-if="!users.data?.length" class="px-6 py-12 text-center">
        ...
        <p class="mt-1 text-xs text-slate-400">Try clearing the search or status filter.</p>
      </div>
    </div>
        ...
        <form class="space-y-4" @submit.prevent="create">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input id="nu-name" v-model="createForm.name" type="text" class="w-full rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40" />
            ...
            <div>
              ...
              <input id="nu-email" v-model="createForm.email" type="email" class="w-full rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40" />
          ...
          <div>
            ...
            <input id="nu-pass" v-model="createForm.password" type="text" class="w-full rounded-lg border-slate-300 text-sm font-mono focus:border-accent-500 focus:ring-accent-500/40" />
          ...
          <div>
            ...
            <div v-for="(assignment, i) in createForm.roles" :key="i" class="flex gap-2 mb-2">
              <select v-model="assignment.role_id" class="flex-1 rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40">
                <option value="">— select role —</option>
                <option v-for="role in roles" :key="role.id" :value="String(role.id)">{{ role.name }}</option>
              </select>
              <select v-model="assignment.project_id" class="flex-1 rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40">
                <option value="">All projects (global)</option>
                <option v-for="p in projects" :key="p.id" :value="String(p.id)">Scoped: {{ p.name }}</option>
              </select>
              <button
                v-if="createForm.roles.length > 1" type="button" class="px-2 text-slate-400 hover:text-red-600"
                aria-label="Remove role assignment" @click="removeRole(i)"
              >
                ✕
              </button>
            </div>
            <button type="button" class="text-xs font-medium text-accent-500 hover:text-accent-600 hover:underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 rounded transition-colors" @click="addRole">+ Add another role</button>
    ...
    <!-- Edit modal -->
    <Modal :show="!!editing" max-width="2xl" @close="editing = null">
        ...
        <form class="space-y-4" @submit.prevent="saveEdit">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input id="eu-name" v-model="editForm.name" type="text" class="w-full rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40" />
            ...
            <div>
              ...
              <input id="eu-email" v-model="editForm.email" type="email" class="w-full rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40" />
          ...
          <div>
            ...
            <input id="eu-pass" v-model="editForm.password" type="text" class="w-full rounded-lg border-slate-300 text-sm font-mono focus:border-accent-500 focus:ring-accent-500/40" />
            <InputError :message="editForm.errors.password" class="mt-1" />
          </div>

          <label class="inline-flex items-center gap-2 text-sm text-slate-700">
            <input
              v-model="editForm.is_active" type="checkbox" :disabled="editing.id === $page.props.auth.user.id"
              class="rounded border-slate-300 text-accent-500 focus:ring-accent-500/40 disabled:opacity-50"
            />
            Account active
          ...
          <div>
            ...
            <div v-for="(assignment, i) in editForm.roles" :key="i" class="flex gap-2 mb-2">
              ...
              <button
                v-if="editForm.roles.length > 1" type="button" class="px-2 text-slate-400 hover:text-red-600"
                aria-label="Remove role assignment" @click="editForm.roles.splice(i, 1)"
              >
                ✕
              </button>
            </div>
            <button
              type="button" class="text-xs font-medium text-accent-500 hover:text-accent-600 hover:underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 rounded transition-colors"
              @click="editForm.roles.push({ role_id: '', project_id: '' })"
            >
              + Add another role
            </button>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2">
            <button type="button" class="px-4 py-2 text-sm text-slate-600 hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 rounded transition" @click="editing = null">Cancel</button>
            <button
    ...
    <Modal :show="!!deleting" max-width="md" @close="deleting = null">
      <div v-if="deleting" class="p-6">
        <h3 class="text-lg font-bold tracking-tight text-slate-900 mb-1">Delete user?</h3>
        <p class="text-sm text-slate-500 mb-4">{{ deleting.email }} — their audit history is preserved.</p>
        <div class="flex justify-end gap-2">
          <SecondaryButton @click="deleting = null">Cancel</SecondaryButton>
          <DangerButton @click="removeUser">Delete user</DangerButton>
        </div>
      </div>
    </Modal>
  </AuthenticatedLayout>
