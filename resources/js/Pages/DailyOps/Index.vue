...
import { IconCalendarCheck, IconCircleCheck, IconCircleMinus, IconCircleX, IconServerOff } from '@tabler/icons-vue';
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between w-full flex-wrap gap-3">
        <h2 class="font-bold text-xl text-slate-900 tracking-tight leading-tight">Daily Ops</h2>
        <form class="flex items-end gap-3" @submit.prevent="applyFilter">
          ...
          <div>
            ...
            <select
              id="ops-project" v-model="filterForm.project_id" class="rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40"
              @change="applyFilter"
            >
          ...
          <div>
            ...
            <select
              id="ops-province" v-model="filterForm.province" class="rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500/40"
              @change="applyFilter"
            >
    ...
    <div class="dict-card p-4 mb-4 sticky top-16 z-10 flex flex-wrap items-center gap-3">
      <span class="text-sm font-medium text-slate-600 tabular-nums" aria-live="polite">
        Reported <span class="text-emerald-700 font-bold">{{ reportedCount }}</span> / {{ counts.total }}
        <span class="sr-only">sites reported today</span>
      </span>
      <button
        type="button" class="inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 bg-white hover:bg-slate-50 border border-slate-300 rounded-lg px-3 py-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 active:scale-[0.98] transition"
      ...
      <div class="ml-auto flex gap-2">
        <button
          type="button" :disabled="!dirtyEntries.length" class="px-4 py-2 rounded-lg text-sm font-medium border border-slate-300 text-slate-600 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 focus-visible:ring-offset-2 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed transition"
          @click="send('save_draft')"
        >
          Save Draft
        </button>
        <button
          type="button" :disabled="!dirtyEntries.length" class="px-4 py-2 rounded-lg text-sm font-medium bg-accent-500 text-white hover:bg-accent-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 focus-visible:ring-offset-2 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed transition"
          @click="send('submit')"
        >
          Submit
        </button>
        <button
          ...
          class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-emerald-700 text-white hover:bg-emerald-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600/40 focus-visible:ring-offset-2 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed transition"
          @click="send('approve')"
        >
        ...
        <div v-for="row in groupRows" :key="row.site_id" class="flex flex-wrap items-center gap-x-4 gap-y-2 px-4 py-2.5 hover:bg-slate-50/60 transition-colors">
          ...
          <template v-else>
            <div class="flex rounded-lg overflow-hidden border border-slate-200" role="group" :aria-label="`Set status for ${row.location_name}`">
              <button
                type="button" class="px-3 py-1.5 text-xs font-semibold transition-colors inline-flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent-500/60"
                :class="rowState(row).status === 'UP' ? 'bg-green-700 text-white' : 'bg-white text-slate-500 hover:bg-green-50'"
              ...
              <button
                type="button" class="px-3 py-1.5 text-xs font-semibold transition-colors border-x border-slate-200 inline-flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent-500/60"
                :class="rowState(row).status === 'DOWN' ? 'bg-red-700 text-white' : 'bg-white text-slate-500 hover:bg-red-50'"
              ...
              <button
                type="button" class="px-3 py-1.5 text-xs font-semibold transition-colors border-r border-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent-500/60"
                :class="rowState(row).status === 'DOWN_SERVER' ? 'bg-orange-700 text-white' : 'bg-white text-slate-500 hover:bg-orange-50'"
                :aria-pressed="rowState(row).status === 'DOWN_SERVER'"
                @click="toggle(row.site_id, 'DOWN_SERVER')"
              >
                <IconServerOff v-if="rowState(row).status === 'DOWN_SERVER'" class="w-3.5 h-3.5" /> SERVER DOWN
              </button>
              <button
                type="button" class="px-3 py-1.5 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent-500/60"
                :class="rowState(row).status === 'NO_NMS' ? 'bg-amber-700 text-white' : 'bg-white text-slate-500 hover:bg-amber-50'"
            ...
            <input
              ...
              class="w-20 rounded-md border-slate-200 text-xs tabular-nums focus:border-accent-500 focus:ring-accent-500/20"
            ...
            <input
              ...
              class="flex-1 min-w-[120px] max-w-[220px] rounded-md border-slate-200 text-xs focus:border-accent-500 focus:ring-accent-500/20"
